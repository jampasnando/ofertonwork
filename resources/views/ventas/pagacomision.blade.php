...
@section('contenido')
        ...
        @endif
        <br>
        <div class="titulo"><h3>Comisiones pendientes de <span style="color:darkorange">{{$nombre}}</span></h3></div>
        <table class="table table-light table-stripped table-hover" id="tb">
            <thead class="thead-light">
                <tr>
                    <th><input type="checkbox" name="chktodo" id="chktodo" onchange="cambiatodo(this)" checked></th>
                    <th>N</th>
                    <th>Descripcion</th>
                    <th>Fecha</th>
                    <th>Deposito</th>
    ...
    function cambiatodo(chktodo){
        ...
        if(chktodo.checked){
            for(j=1;j<nrofilas;j++){
                tb.rows[j].cells[0].querySelector("input").checked=true;
            }
        }
        else{
            for(j=1;j<nrofilas;j++){
                tb.rows[j].cells[0].querySelector("input").checked=falses;
            }
        }
        totaliza();
    }
    function totaliza(){
        ...
        for(k=1;k<nrofilas;k++){
            if(document.getElementById("ch_"+k).checked){
                total=total + parseFloat(tb.rows[k].cells[9].innerText);
            }
        }
